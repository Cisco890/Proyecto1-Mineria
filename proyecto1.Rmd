...
violencia %>%
  ...
  labs(title = "Estacionalidad de la Violencia Intrafamiliar",
       x = "Mes del Anno (1 = Enero, 12 = Diciembre)",
...
```{r}
divorcios %>%
  filter(!is.na(ESCHOM), !is.na(ESCMUJ)) %>%
  group_by(ESCHOM, ESCMUJ) %>%
  count() %>%
  ggplot(aes(x = factor(ESCHOM), y = factor(ESCMUJ), fill = n)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "red") +
  labs(title = "Mapa de Calor: Nivel Educativo en Divorcios",
       subtitle = "Codigos: 1=Ninguno ... 6+=Superior (Verificar diccionario)",
