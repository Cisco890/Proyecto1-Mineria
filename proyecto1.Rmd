...
# Violencia: víctima vs agresor
violencia %>%
  mutate(agr_edad_clean = ifelse(is.na(agr_edad) | agr_edad > 110, NA, agr_edad)) %>%
  ggplot(aes(x = vic_edad_clean, y = agr_edad_clean)) +
  geom_point(alpha = 0.25) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title="Edad de víctima vs agresor",
       x="Edad víctima (limpia)", y="Edad agresor (limpia)") +
  theme_classic()
